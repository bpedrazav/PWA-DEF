...
// =========================================================================
// 4. ENDPOINT DE METADATA (TEMPORADAS Y EPISODIOS REALES)
// =========================================================================
app.get('/api/tv-metadata/:id', async (req, res) => {
  try {
    const url = `https://api.themoviedb.org/3/tv/${req.params.id}?api_key=${API_KEY}&language=es-MX`;
    const response = await axios.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 6000 });
    if (response.data && response.data.seasons) {
      const seasons = response.data.seasons
        .map(s => ({ season_number: s.season_number, episode_count: s.episode_count }))
        .filter(s => s.season_number > 0 && s.episode_count > 0); // Omitir especiales y temp. vacías
      return res.json({ success: true, seasons });
    }
    throw new Error("Sin temporadas");
  } catch (error) {
